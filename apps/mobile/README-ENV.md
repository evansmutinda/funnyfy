...
See `MD/SENTRY_INTEGRATION.md` for testing and optional `EXPO_PUBLIC_SENTRY_TEST`.
...
`SENTRY_ORG` and `SENTRY_PROJECT` are set in `eas.json` (`funnyfy` / `react-native`). Also set `EXPO_PUBLIC_SENTRY_DSN` (and related `EXPO_PUBLIC_SENTRY_*` vars) as EAS secrets if not already — see `MD/SENTRY_INTEGRATION.md`.
