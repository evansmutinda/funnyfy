...
| Admin | `<env-url>/admin/login` — `admin_users` / `MD/FUNNYFY_FLOW.md` |
...
**Setup:** `admin_users` (or legacy `ADMIN_USER_IDS`) in Vercel — see `MD/FUNNYFY_FLOW.md`
...
| `(NOBRIDGE) ERROR API error: GenerationFailedError` | Job `failed` on server (usually upstream 5xx). Metro `[API error detail]` → `rawErrorMessage`; check [Replicate Status](https://status.replicate.com/). Users see “Image generation is unavailable at the moment…” for 5xx / internal server error JSON |
...
- [ ] Admin login works; Finance + Growth pages show data (`MD/FUNNYFY_FLOW.md`)
